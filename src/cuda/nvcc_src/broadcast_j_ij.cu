...
__global__ void __kernel_broadcast_j_ij(
  ...
  for (len_t m_step = 0; m_step < m; m_step += (gridDim.y * blockDim.y))
  {
    ...
    for (len_t n_step = 0; n_step < n; n_step += blockDim.x) {
      ...
      if ((m_pos < m) && (n_pos < n)) {
        dst[m_pos * n + n_pos] = src[n_pos] + alpha * dst[m_pos * n + n_pos];
      }
    }
  }
}

extern "C" void launch_broadcast_j_ij_Scalar(
  const void* src, 
        void* dst, 
  double alpha,
  len_t m,
  ...
  const dim3 grid(1ul, std::min(12ul, DIMPAD(m, WARP_SIZE)));
  const dim3 block(WARP_SIZE, WARP_SIZE);

  __kernel_broadcast_j_ij<<<grid, block, 0, get_stream(stream)>>>(
