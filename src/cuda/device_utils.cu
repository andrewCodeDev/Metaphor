...
extern "C" void* mpMemAlloc(size_t N, StreamCtx stream) {
  CUstream _stream = static_cast<CUstream>(stream.ptr);
  CUdeviceptr dptr;
  CURESULT_ASSERT(cuMemAllocAsync(&dptr, N, _stream));
  return (void*)dptr;
}
extern "C" void mpMemcpyHtoD(void* dev_ptr, const void* cpu_ptr, size_t N, StreamCtx stream) {
  CUstream _stream = static_cast<CUstream>(stream.ptr);
  CUdeviceptr dptr = reinterpret_cast<CUdeviceptr>(dev_ptr);
  CURESULT_ASSERT(cuMemcpyHtoDAsync(dptr, cpu_ptr, N, _stream));
}
extern "C" void mpMemcpyDtoH(void* cpu_ptr, void const* dev_ptr, size_t N, StreamCtx stream) {
  CUstream _stream = static_cast<CUstream>(stream.ptr);
  CUdeviceptr dptr = reinterpret_cast<CUdeviceptr>(dev_ptr);
