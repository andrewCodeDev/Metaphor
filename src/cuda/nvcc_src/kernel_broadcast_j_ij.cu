...
__global__ void __kernel_broadcast_j_ij_RScalar(
...
extern "C" void launch_broadcast_j_ij_RScalar(
  StreamCtx stream, 
  const RScalar* src, 
        RScalar* dst, 
        RScalar alpha,
  ...
  __kernel_broadcast_j_ij_RScalar<<<grid_block, thread_block, 0, getCtx(stream)>>>(src, dst, alpha, m, n);
}
