...
extern "C" void launch_relu_leaky_reverse_r16(
  ...
  __kernel_leaky_relu_reverse_r16<<<1, GRID_1D(N), 32, getStream(stream)>>>(
...
extern "C" void launch_relu_leaky_reverse_r32(
  ...
  __kernel_leaky_relu_reverse_r32<<<1, GRID_1D(N), 32, getStream(stream)>>>(
    a_value, a_grads, b_grads, coef, N
  );
}
#include "../kernel_header.h"
...
extern "C" void launch_relu_leaky_reverse_r64(
  ...
  __kernel_leaky_relu_reverse_r64<<<1, GRID_1D(N), 32, getStream(stream)>>>(
    a_value, a_grads, b_grads, coef, N
  );
}
