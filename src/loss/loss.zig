...
const Key = @import("algorithm.zig").Key;
...
pub fn contrast(
    ...
    const x_grads = if (x.grads()) |grads| grads else x.values();
    const y_grads = if (y.grads()) |grads| grads else y.values();

    overloads.kernel_contrastive_loss.call(.{
        stream.context,
        x_value.ptr,
        x_grads.ptr,
        y_value.ptr,
    ...
    if (cpu_score) |cpu_ptr| {
        DU.copyFromDevice(gpu_score.?, @ptrCast(@alignCast(cpu_ptr)), stream);
